...
static int8_t GarminJoystik_Init     (void);
static int8_t GarminJoystik_DeInit   (void);
static int8_t GarminJoystik_Control  (uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t GarminJoystik_Receive  (uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_GarminJoystik_fops = 
{
  GarminJoystik_Init,
  GarminJoystik_DeInit,
  GarminJoystik_Control,
  GarminJoystik_Receive
};
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t GarminJoystik_Receive(uint8_t* Buf, uint32_t *Len)
{
